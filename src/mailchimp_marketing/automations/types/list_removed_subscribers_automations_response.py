# This file was auto-generated from our API Definition.

import typing

import pydantic
import typing_extensions
from ...core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from ...core.serialization import FieldMetadata
from ...types.subscriber_removed_from_automation_workflow import SubscriberRemovedFromAutomationWorkflow
from .list_removed_subscribers_automations_response_links_item_item import (
    ListRemovedSubscribersAutomationsResponseLinksItemItem,
)


class ListRemovedSubscribersAutomationsResponse(UniversalBaseModel):
    """
    A summary of the subscribers who were removed from an Automation workflow.
    """

    links: typing_extensions.Annotated[
        typing.Optional[typing.List[typing.List[ListRemovedSubscribersAutomationsResponseLinksItemItem]]],
        FieldMetadata(alias="_links"),
        pydantic.Field(
            alias="_links", description="A list of link types and descriptions for the API schema documents."
        ),
    ] = None
    """
    A list of link types and descriptions for the API schema documents.
    """

    subscribers: typing.Optional[typing.List[SubscriberRemovedFromAutomationWorkflow]] = pydantic.Field(default=None)
    """
    An array of objects, each representing a subscriber who was removed from an Automation workflow.
    """

    total_items: typing.Optional[int] = pydantic.Field(default=None)
    """
    The total number of items matching the query regardless of pagination.
    """

    workflow_id: typing.Optional[str] = pydantic.Field(default=None)
    """
    A string that uniquely identifies an Automation workflow.
    """

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
