# This file was auto-generated from our API Definition.

import typing

import pydantic
import typing_extensions
from ...core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from ...core.serialization import FieldMetadata
from ...types.subscriber_list import SubscriberList
from .list_lists_response_constraints import ListListsResponseConstraints
from .list_lists_response_links_item import ListListsResponseLinksItem


class ListListsResponse(UniversalBaseModel):
    """
    A collection of subscriber lists for this account.
    """

    links: typing_extensions.Annotated[
        typing.Optional[typing.List[ListListsResponseLinksItem]],
        FieldMetadata(alias="_links"),
        pydantic.Field(
            alias="_links", description="A list of link types and descriptions for the API schema documents."
        ),
    ] = None
    """
    A list of link types and descriptions for the API schema documents.
    """

    constraints: typing.Optional[ListListsResponseConstraints] = pydantic.Field(default=None)
    """
    Do particular authorization constraints around this collection limit creation of new instances?
    """

    lists: typing.List[SubscriberList] = pydantic.Field()
    """
    An array of objects, each representing a list.
    """

    total_items: typing.Optional[int] = pydantic.Field(default=None)
    """
    The total number of items matching the query regardless of pagination.
    """

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
